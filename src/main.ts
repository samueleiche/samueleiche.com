...
const el = document.querySelector('.logo') as HTMLElement
const paths = Array.from(el.querySelectorAll('path'))
let isPointerDown = false

const animation = anime({
	targets: paths,
	d: (el: SVGPathElement, i: number): string => SVGMorphPaths[i],
...
elementEvents.forEach((event) => el.addEventListener(event, handleEvent, false))
windowEvents.forEach((event) => window.addEventListener(event, handleEvent, false))
